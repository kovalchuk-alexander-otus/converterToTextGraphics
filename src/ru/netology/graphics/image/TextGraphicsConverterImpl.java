...
public class TextGraphicsConverterImpl implements TextGraphicsConverter {
    ...
    final int I_HEIGHT = 0;
    final int I_WIDTH = 1;
    int maxWidth; // максимальная ширина результирующего изображения в "текстовых пикселях"
    int maxHeight; // максимальная высота результирующего изображения в "текстовых пикселях"
    double maxRatio; // максимально допустимое соотношение сторон исходного изображения
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        System.out.printf("Ширина [%d], длина [%d]%n%n", img.getWidth(), img.getHeight());
        checkRatio(img.getHeight(), img.getWidth()); // проверим соответствие разрешения картинки заданному
        int[] newSize = geRevisedSize(((BufferedImage) img).getHeight(), img.getWidth()); // получим размер картинки в пропорции с учетом ограничений
        System.out.printf("Уточненная ширина [%d], длина [%d]%n%n", newSize[I_WIDTH], newSize[I_HEIGHT]);
        ...
        Image scaledImage = img.getScaledInstance(newSize[I_WIDTH], newSize[I_HEIGHT], BufferedImage.SCALE_SMOOTH);
        ...
        BufferedImage bwImg = new BufferedImage(newSize[I_WIDTH], newSize[I_HEIGHT], BufferedImage.TYPE_BYTE_GRAY);
        ...
        return convertToString(bwImg);
    }

    // проверка соответствия разрешения картинки заданному ..если его задали
    // TODO: ??? интересно, тут более уместно создать метод с двумя параметрами width и height
    private void checkRatio(double height, double width) throws BadImageSizeException {
        ...
        if (this.maxRatio != 0) {
            System.out.println(this.maxRatio);
            double realRatio = height > width ? height / width : width / height;
            System.out.println(realRatio);
            if (realRatio > this.maxRatio) {
                throw new BadImageSizeException(realRatio, this.maxRatio);
            }
        }
    }

    private int[] geRevisedSize(int height, int width) {
        ...
        double ratio, ratioHeight = 0, ratioWidth = 0;
        if (this.maxHeight != 0) {
            ratioHeight = (double) height / this.maxHeight;
        }
        if (this.maxWidth != 0) {
            ratioWidth = (double) width / this.maxWidth;
        }
        System.out.println(ratioHeight);
        System.out.println(ratioWidth);
        ratio = Math.max(ratioHeight, ratioWidth);
        if (ratio != 0) {
            newHeight /= ratio;
            newWidth /= ratio;
        }
        return new int[]{(int) Math.round(newHeight), (int) Math.round(newWidth)};
    }

    private String convertToString(BufferedImage img) {
     ...
     *
     * @param maxWidth максимальная ширина текстовых картинок
     */
    @Override
     ...
     * с сохранением пропорций.
     *
     * @param maxHeight максимальная высоту текстовых картинок
     */
    @Override
